...
class DatabaseManager:
    ...
    def _initialize_database(self):
        """Create database tables from schema file or embedded schema."""
        import sys
        schema_paths = [
            os.path.join(os.path.dirname(__file__), 'schema.sql'),
        ]
        if getattr(sys, 'frozen', False):
            schema_paths.append(os.path.join(sys._MEIPASS, 'database', 'schema.sql'))
        ...
        if not schema_sql:
            # Embedded fallback DDL to guarantee database tables always exist
            schema_sql = """
