...
class SiftEngine:
    ...
    def _verify_imports_with_pypi(self, code_content: str) -> str:
        ...
        results = []
        # Standard library whitelist (simplified for research speed)
        std_lib = {
        ...
        for pkg in imports:
            ...
            try:
                # Fast 1-second timeout check to PyPI
    ...
    def analyze_code(self, code_content: str, filename: str = None, **kwargs) -> dict:
        ...
        Updated to accept 'filename' and other kwargs to prevent UI crashes.
        """
        try:
            # Step 1: Detect Language
            language = self.detect_language(code_content)
            
            # Step 2: Real-Time Registry Verification
            registry_context = ""
            if language.lower() in ['python', 'python 3']:
                registry_context = self._verify_imports_with_pypi(code_content)

            # Step 3: Construct Chain-of-Thought System Prompt
            system_prompt = """
            ...
            {
                "analysis_steps": "Step 1: Checked imports... Step 2: Analyzed logic...",
