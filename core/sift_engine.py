...
class SiftEngine:
    ...
    def analyze_code(self, code_content: str, filename: str = None, **kwargs) -> dict:
        ...
        try:
            ...
            system_prompt = (
                "You are Sift, a precise code security auditor. "
                "Analyze the provided code for REAL, EXPLOITABLE vulnerabilities only. "
                "\n\n"
                "CRITICAL — FALSE-POSITIVE AVOIDANCE RULES:\n"
                "- Only flag a vulnerability if it is genuinely exploitable in the given context. "
                "Do NOT flag theoretical or hypothetical risks that require unlikely conditions.\n"
                "- Do NOT flag standard library usage (os, sys, json, math, re, logging, subprocess, etc.) "
                "as suspicious unless it is used in a demonstrably dangerous way (e.g., subprocess with shell=True "
                "AND unsanitized user input).\n"
                "- Do NOT flag well-known safe patterns as vulnerabilities: parameterized SQL queries, "
                "ORM usage, framework-standard patterns (app.route, express.Router), input validation, "
                "or proper error handling.\n"
                "- Do NOT flag code style, naming conventions, missing docstrings, or best-practice suggestions "
                "as vulnerabilities. Only security issues belong in the vulnerabilities list.\n"
                "- If the code is genuinely safe, return an empty vulnerabilities list and a score of 100. "
                "Do not manufacture issues where none exist.\n"
                "\n"
                "WHAT TO FLAG:\n"
                ...
                "- Hallucinated/non-existent imports (use the REAL-TIME REGISTRY CONTEXT)\n"
                "- Cross-site scripting (XSS) — rendering user input without escaping\n"
                ...
                "\n\n"
                "SCORING RULES:\n"
                "- 90-100: Code is safe or has only trivial, non-exploitable observations\n"
                "- 70-89: Minor issues that could become risks under specific conditions\n"
                "- 40-69: Moderate vulnerabilities that should be addressed\n"
                "- 0-39: Critical, easily exploitable vulnerabilities present\n"
                "\n"
